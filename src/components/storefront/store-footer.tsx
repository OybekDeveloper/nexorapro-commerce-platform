  ...
  return (
    <footer className="border-t border-black/5 bg-[#f5f5f7] px-4 py-12 text-sm text-zinc-600 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[1.2fr_2fr]">
        <div><Link href="/" className="inline-flex cursor-pointer items-center gap-2 text-lg font-semibold tracking-[-0.04em] text-[#1d1d1f]"><NexoraMark className="size-7 text-brand" />nexorapro<span className="-ml-2 text-brand">.uz</span></Link><p className="mt-4 max-w-sm leading-6">{labels.intro}</p></div>
        <div className="grid grid-cols-2 gap-8 sm:grid-cols-3"><div><p className="font-semibold text-[#1d1d1f]">{labels.shop}</p><div className="mt-3 space-y-2"><Link href="/catalog" className="block hover:text-brand">{labels.catalog}</Link><Link href="/cart" className="block hover:text-brand">{labels.cart}</Link></div></div><div><p className="font-semibold text-[#1d1d1f]">{labels.categories}</p><div className="mt-3 space-y-2"><Link href="/catalog?category=Smartfon" className="block hover:text-brand">{labels.phones}</Link><Link href="/catalog?category=Noutbuk" className="block hover:text-brand">{labels.laptops}</Link><Link href="/catalog?category=Planshet" className="block hover:text-brand">{labels.tablets}</Link></div></div><div><p className="font-semibold text-[#1d1d1f]">{labels.platform}</p><div className="mt-3 space-y-2"><Link href="/admin" className="block hover:text-brand">{labels.admin}</Link><span className="block">UZ · RU · EN</span></div></div></div>
      </div>
