...
type OrderStatus =
  "new" | "paid" | "packing" | "shipping" | "completed" | "cancelled";
type Order = {
  id: string;
  customer: string;
  phone: string;
...
function toDisplayOrder(order: CommerceOrder): Order {
  return {
    ...
    items: order.items
      .map((item) => `${item.productName} · ${item.quantity} dona`)
      .join(", "),
...
export function OrdersWorkspace() {
    ...
    () =>
      orders.filter(
        (order) =>
          ...
          `${order.id} ${order.customer} ${order.phone} ${order.items}`
            .toLowerCase()
  ...
  useEffect(() => {
    apiRequest<{ orders: CommerceOrder[] }>("/api/orders")
      ...
      .catch((cause) =>
        setError(
          cause instanceof Error ? cause.message : "Buyurtmalar yuklanmadi",
        ),
      )
  ...
  const updateStatus = async (id: string, next: OrderStatus) => {
    setError(null);
    try {
      const payload = await apiRequest<{ order: CommerceOrder }>(
  ...
  return (
    <div className="space-y-5">
      {error && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/[0.06] px-4 py-3 text-sm text-red-700">
          {error}
        </div>
      )}
        ...
        <div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative">
            <select
              ...
              className="h-10 w-full cursor-pointer appearance-none rounded-xl border border-input bg-background pl-3 pr-9 text-sm font-medium outline-none focus:ring-2 focus:ring-ring"
            >
          ...
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="bg-muted/60 text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">Tarkib</th>
                  ...
                  <td className="px-4 py-4">
                    {nextStatus[order.status] ? (
                      <button
                        ...
                        className="h-8 cursor-pointer rounded-lg bg-brand px-3 text-xs font-semibold text-white hover:opacity-85"
                      >
                        Keyingi status
                      </button>
                    ) : (
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between text-sm">
                <span className="text-muted-foreground">
                  {order.channel} · {order.payment}
                </span>
                <strong>{order.total} UZS</strong>
              </div>
            </button>
          ))}
        </div>
        {filtered.length === 0 && (
          <div className="px-5 py-16 text-center">
            ...
            <p className="mt-3 font-medium">
              {loading ? "Buyurtmalar yuklanmoqda..." : "Buyurtma topilmadi"}
            </p>
          </div>
        )}
        <div className="border-t border-border px-5 py-3 text-xs text-muted-foreground">
          {filtered.length} ta buyurtma · persistent API ma’lumoti
        </div>
      </section>
      {selected && (
        <div className="fixed inset-0 z-[70]">
          ...
          />
          <aside className="absolute inset-y-0 right-0 w-[min(92vw,430px)] overflow-y-auto bg-background p-5 shadow-2xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-brand">
                  Buyurtma tafsiloti
                </p>
                <h2 className="mt-1 text-xl font-semibold">{selected.id}</h2>
              </div>
              <button
                type="button"
                onClick={() => setSelected(null)}
                className="inline-flex size-9 cursor-pointer items-center justify-center rounded-xl bg-muted"
                aria-label="Yopish"
              >
                <XCircle className="size-5" />
              </button>
            </div>
            <div className="mt-6 space-y-3 rounded-2xl border border-border p-4 text-sm">
              <div>
              ...
              <div className="border-t border-border pt-3">
                ...
                <p className="mt-1 font-medium">{selected.items}</p>
              </div>
              <div className="border-t border-border pt-3">
            ...
            <label className="mt-5 block space-y-2">
              <span className="text-sm font-semibold">Statusni boshqarish</span>
              <select
                ...
                className="h-11 w-full cursor-pointer rounded-xl border border-input bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring"
              >
                {Object.entries(statusMeta).map(([value, meta]) => (
                  <option key={value} value={value}>
                    {meta.label}
            ...
            <div className="mt-5 rounded-2xl bg-muted p-4">
              <p className="text-sm font-semibold">Buyurtma timeline</p>
              <div className="mt-3 space-y-3 text-xs text-muted-foreground">
                <p>• Buyurtma {selected.time} da qabul qilindi</p>
                <p>• To‘lov usuli: {selected.payment}</p>
