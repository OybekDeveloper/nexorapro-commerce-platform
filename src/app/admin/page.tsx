...
export default async function AdminDashboardPage() {
  ...
  const revenueData = salesReport.daily.map((item) => ({ date: item.date.slice(5), revenue: Number((item.revenue / 1_000_000).toFixed(2)) }));
  const categoryData = salesReport.categories.map((item) => ({ name: item.name, value: item.revenue }));
  const categoryTotal = categoryData.reduce((sum, item) => sum + item.value, 0);
  const leadingCategory = categoryData[0];
  const leadingShare = leadingCategory && categoryTotal > 0 ? Math.round((leadingCategory.value / categoryTotal) * 100) : 0;
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.65fr)_minmax(320px,0.75fr)]">
        <article className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
          <div className="mb-5 flex items-start justify-between">
            ...
            <Link href="/admin/analytics" className="inline-flex h-9 items-center rounded-xl border border-border bg-background px-3 text-sm font-medium hover:bg-muted">Batafsil</Link>
          </div>
        ...
        <article className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-2 rounded-xl bg-muted px-3 py-2 text-xs text-muted-foreground">{leadingCategory ? <>{leadingCategory.name} jami tushumning <strong className="text-foreground">{leadingShare}%</strong>ini tashkil qildi.</> : "Tanlangan davrda sotuv hali yo‘q."}</div>
        </article>
      </section>
